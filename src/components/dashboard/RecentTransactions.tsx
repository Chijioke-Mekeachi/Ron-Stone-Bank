  ...
  const fetchRecentTransactions = async () => {
    try {
      ...
      const API_BASE_URL = process.env.NODE_ENV === 'development' 
        ? 'https://renostarbank.onrender.com/api'
        : '/api';

      const response = await fetch(`${API_BASE_URL}/transactions?limit=8`, {
        headers: {
      ...
      if (error instanceof Error) {
        if (error.message.includes('HTTP error! status: 401')) {
          ...
          toast.error('Transactions endpoint not found. Please check API configuration.');
        } else if (error.message.includes('non-JSON')) {
          toast.error('Server configuration error. Please try again later.');
        } else {
          toast.error('Failed to load transactions. Please check your connection.');
        }
      } else {
